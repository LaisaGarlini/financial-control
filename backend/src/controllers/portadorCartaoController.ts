...
class PortadorCartaoController {
    async listar(req: Request, res: Response): Promise<void> {
        try {
            const portadores_cartoes = await PortadorCartaoRepository.GetPortadoresCartao()
    ...
    async buscarPorId(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params
            const portadores_cartoes = await PortadorCartaoRepository.GetPortadorCartaoById(+id)
    ...
    async atualizar(req: Request, res: Response): Promise<void> {
        try {
            ...
            const portadores_cartoes = await PortadorCartaoRepository.Update({ id: +id, usuario_id, portador_id, cartao_id })
            if (!portadores_cartoes) {
                res.status(404).json({ message: `Portador cartão com ID ${id} não encontrado.` })
                return
            }
    ...
    async deletar(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params
            const portadores_cartoes = await PortadorCartaoRepository.Delete(+id)
            if (!portadores_cartoes) {
                res.status(404).json({ message: `Portador cartão com ID ${id} não encontrado.` })
                return
            }
