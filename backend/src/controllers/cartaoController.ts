...
class CartaoController {
    async listar(req: Request, res: Response): Promise<void> {
        try {
            const cartoes = await CartaoRepository.GetCartoes()
            if (cartoes.length === 0) {
    ...
    async buscarPorId(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params
            const cartoes = await CartaoRepository.GetCartaoById(+id)
            if (!cartoes) {
    ...
    async criar(req: Request, res: Response): Promise<void> {
        try {
            ...
            const novoCartao = await CartaoRepository.Create({ usuario_id, conta_financeira_id, apelido, tipo, ativo })
            res.status(201).json({
    ...
    async atualizar(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!usuario_id && !conta_financeira_id && !apelido && !tipo && !ativo === undefined) {
                res.status(400).json({
                    message:
                        'Pelo menos um dos campos "usuario_id", "conta_financeira_id", "apelido", "tipo" ou "ativo" deve ser fornecido.',
                })
                return
            }

            const cartao = await CartaoRepository.Update({ id: +id, usuario_id, conta_financeira_id, apelido, tipo, ativo })
            if (!cartao) {
    ...
    async deletar(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params
            const cartaoRemovido = await CartaoRepository.Delete(+id)
            if (!cartaoRemovido) {
                res.status(404).json({ message: `Cartão com ID ${id} não encontrado.` })
                return
            }
